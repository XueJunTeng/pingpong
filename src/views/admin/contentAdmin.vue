<template>
  <div class="management-container">
    ...
    <el-dialog
      ...
      width="500px"
    >
      <span v-if="deleteMode === 'single'">确定要永久删除该内容吗？</span>
      <span v-else>确定要永久删除选中的 {{ selectedIds.length }} 条内容吗？</span>
...
const handleSingleDelete = (row: ContentItem) => {
  selectedIds.value = [row.contentId]
...
const confirmDelete = async () => {
  try {
    if (deleteMode.value === 'single') {
      await auditStore.deleteContent(selectedIds.value[0])  // 直接调用你的删除方法
    } else {
      await auditStore.batchDeleteContents(selectedIds.value)  // 调用你的批量删除方法
    }

    ElMessage.success('删除成功')
    auditStore.fetchContentsList()  // 保持原有刷新逻辑
    selectedIds.value = []
  } catch (error) {
